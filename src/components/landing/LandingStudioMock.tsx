...
export function LandingStudioMock() {
  return (
    <div className="flex h-full min-h-[420px] w-full overflow-hidden rounded-[calc(0.75rem-2px)] bg-[var(--landing-surface)] lg:min-h-[520px]">
      <aside className="hidden w-[220px] shrink-0 flex-col gap-2 border-r border-landing bg-[var(--landing-surface-2)] p-3 lg:flex">
        <div className="flex items-center gap-2 px-2 py-1">
          <SysDesLogoIcon className="h-7 w-7 opacity-90" title={BRAND.name} />
          <span className="text-sm font-semibold text-[var(--landing-fg)]">{BRAND.name}</span>
        </div>
        <nav className="mt-2 flex flex-col gap-0.5">
          {SIDEBAR.map(({ Icon, label }) => (
            <span
              key={label}
              className="flex items-center gap-2 rounded-md px-2 py-2 text-sm text-[var(--landing-fg)] hover:bg-black/5 dark:hover:bg-white/5"
            >
        ...
        <Link
          href="/studio"
          className="mt-auto flex items-center justify-center gap-1 rounded-lg border border-landing px-3 py-2.5 text-sm font-semibold text-[var(--landing-fg)] transition-colors hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black"
        >
          Open studio
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="flex items-center justify-between gap-2 border-t border-landing bg-[var(--landing-surface-2)] px-4 py-2.5">
