  ...
  return (
    <Tabs value={activeRightTab} onValueChange={(v) => setActiveRightTab(v as typeof activeRightTab)} className="flex flex-1 flex-col min-h-0">
      <div className="mx-2 mt-2 shrink-0 overflow-x-auto">
        <TabsList className="h-auto min-h-9 w-max gap-0.5 bg-zinc-200 p-1 dark:bg-zinc-800">
          <TabsTrigger
            value="properties"
...
export function RightPanel({ open = true, onSimulate, variant = "desktop" }: RightPanelProps) {
  ...
  return (
    <aside
      className={`hidden shrink-0 flex-col border-l border-zinc-200 bg-zinc-50 overflow-hidden transition-all duration-200 dark:border-zinc-800 dark:bg-zinc-900 md:flex ${
      ...
      <div className="space-y-2">
        ...
        <div>
          ...
          <input
            ...
            className="w-full rounded-md border border-input bg-background px-2.5 py-1.5 text-xs text-foreground placeholder:text-muted-foreground outline-none focus:border-cyan-600 focus:ring-1 focus:ring-cyan-600/50"
          />
        </div>
        ...
        <div>
          ...
          <select
            ...
            className="w-full rounded-md border border-input bg-background px-2.5 py-1.5 text-xs text-foreground outline-none focus:border-cyan-600 focus:ring-1 focus:ring-cyan-600/50"
          >
            {protocols.map((p) => (
          ...
          <div className="flex gap-1">
            <button
              ...
              className={`flex-1 rounded-md border px-2.5 py-1.5 text-xs font-medium transition-colors ${
                !data.async
                  ? "border-cyan-500/30 bg-cyan-600/20 text-cyan-600 dark:text-cyan-400"
    ...
    <div className="space-y-4">
      ...
      {selectedNode && selectedNode.type === "text" ? (
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <div className="rounded-md border border-border bg-card px-3 py-2">
              <p className="text-xs font-medium text-foreground">Text Note</p>
              <p className="mt-0.5 text-xs text-muted-foreground">Double-click on canvas to edit</p>
            </div>
            <Button
              variant="outline"
              size="sm"
        ...
        <div className="flex flex-col items-center gap-3 py-6 text-center">
          ...
          <div>
            <p className="text-xs font-medium text-foreground">No component selected</p>
        ...
        {hints.map((hint, i) => (
          ...
          >
            ...
            >
              ...
              <span className="flex-1 text-xs font-medium text-foreground">{hint.title}</span>
              {expandedHints.has(i) ? (
            ...
            {expandedHints.has(i) && (
              <div className="border-t border-border px-2.5 py-2">
                <p className="text-xs leading-relaxed text-muted-foreground">{hint.content}</p>
              </div>
            )}
          </div>
        ))}
...
function LearnSection({ componentId, label }: { componentId: string; label: string }) {
  ...
  return (
    <div className="space-y-2">
      ...
      >
        <BookOpen className="h-3.5 w-3.5 shrink-0 text-cyan-600 dark:text-cyan-400" />
        <span className="flex-1 text-xs font-medium text-foreground">Learn about {label}</span>
        {expanded ? (
          <ChevronDown className="h-3 w-3 shrink-0 text-muted-foreground" />
        ) : (
          <ChevronRight className="h-3 w-3 shrink-0 text-muted-foreground" />
