...
/** One line per built-in component for AI prompts (`componentId` — Label). */
export function getBuiltinComponentCatalogText(): string {
  return SYSTEM_COMPONENTS.map((c) => `${c.id} — ${c.label}`).join("\n");
}
